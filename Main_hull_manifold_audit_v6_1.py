...
def _make_geometry_descriptor_table():
    """Publication-facing definition table for the 16 geometry descriptors."""
    interpretations = {
        "Cp": "Prismatic coefficient",
        "Cm": "Midship coefficient",
        "Cb": "Block coefficient",
        "Cw": "Waterplane coefficient",
        "Lwl/Bwl": "Length-to-beam ratio",
        "Bwl/Tc": "Beam-to-draft ratio",
        "Lwl/Tc": "Length-to-draft ratio",
        "Lwl/Vol^(1/3)": "Length-to-volume scale ratio",
        "Lcb/Lwl": "Longitudinal center-of-buoyancy ratio",
        "Lcf/Lwl": "Longitudinal center-of-flotation ratio",
        "Lcb/Lcf": "Relative longitudinal-center ratio",
        "Sc/Vol^(2/3)": "Wetted-surface area scaling",
        "Aw/Vol^(2/3)": "Waterplane-area scaling",
        "Sc/Aw": "Wetted-surface to waterplane-area ratio",
        "Sc/Ax": "Wetted-surface to reference-area ratio",
        "Ax/Aw": "Reference-area to waterplane-area ratio",
    }
    rows = []
    for i, c in enumerate(GEOMETRY_COLUMNS, 1):
        rows.append({
            "No": i,
            "Descriptor": c,
            "Physical_interpretation": interpretations.get(c, "Geometric descriptor"),
        })
    return pd.DataFrame(rows)


def _make_intrinsic_dimension_table(intrinsic_df):
    """Compact main-text table of all primary ID diagnostics."""
    out = intrinsic_df.copy()
    out["Estimate"] = pd.to_numeric(out["estimate"], errors="coerce")
    out = out[["method", "parameter", "Estimate", "n_samples", "dimension"]].copy()
    out.columns = ["Estimator", "Parameter", "Estimate", "N_hulls", "Nominal_dimension"]
    return out


def _make_series_id_table(series_df):
    """Wide series-level ID table suitable for a main-text table."""
    if series_df is None or series_df.empty:
        return pd.DataFrame()
    s = series_df.copy()
    s["k"] = s["parameter"].astype(str).str.extract(r"k\s*=\s*([0-9]+)")[0]
    s["estimate"] = pd.to_numeric(s["estimate"], errors="coerce")
    rows = []
    for dataset, g in s.groupby("dataset", sort=True):
        row = {"Series": dataset, "N_hulls": int(g["n_samples"].iloc[0])}
        for k in ["5", "7", "10", "12", "15", "20"]:
            vals = g.loc[(g["method"] == "Local MLE") & (g["k"] == k), "estimate"]
            row[f"MLE_k{k}"] = float(vals.iloc[0]) if len(vals) else np.nan
        vals = g.loc[g["method"] == "Two-NN", "estimate"]
        row["TWO_NN"] = float(vals.iloc[0]) if len(vals) else np.nan
        rows.append(row)
    return pd.DataFrame(rows)


def _make_robustness_table(jackknife_summary, subsample_summary, lvo_df):
    """Unified numerical robustness table: jackknife, subsampling, LVO."""
    rows = []
    if jackknife_summary is not None and not jackknife_summary.empty:
        for _, r in jackknife_summary.iterrows():
            rows.append({
                "Analysis": "Leave-one-hull-out jackknife",
                "Estimator": r["method"],
                "Condition": "70 fits",
                "Median": r["median"],
                "Q2.5": r["q025"],
    ...
    if subsample_summary is not None and not subsample_summary.empty:
        for _, r in subsample_summary.iterrows():
            rows.append({
                "Analysis": "Subsampling without replacement",
                "Estimator": r["method"],
                "Condition": r["dataset"].replace("SUBSAMPLE_", ""),
                "Median": r["median"],
                "Q2.5": r["q025"],
                "Q97.5": r["q975"],
                "Min": np.nan,
    ...
    if lvo_df is not None and not lvo_df.empty:
        for col in ["MLE_k10", "MLE_k15", "TWO_NN"]:
            vals = pd.to_numeric(lvo_df[col], errors="coerce").dropna()
            if len(vals):
                rows.append({
                    "Analysis": "Leave-one-variable-out",
                    "Estimator": col,
                    "Condition": f"{len(vals)} omitted descriptors",
                    "Median": float(vals.median()),
                    "Q2.5": float(vals.quantile(0.025)),
                    "Q97.5": float(vals.quantile(0.975)),
                    "Min": float(vals.min()),
                    "Max": float(vals.max()),
                })
    return pd.DataFrame(rows)
...
def _run_hydrodynamic_mapping_cv(df, hull_df, X_hull, n_splits=5, random_state=42):
    """
    Hull-level 5-fold CV for hydrodynamic response mapping.

    Baseline: Fn + Rn
    3D:       Fn + Rn + Isomap(3D)
    4D:       Fn + Rn + Isomap(4D)

    Isomap is fitted within each training fold and then transformed on the
    held-out hulls, avoiding use of test-hull geometry to fit the manifold.
    The same RandomForest regressor is used for all feature sets.
    """
    from sklearn.ensemble import RandomForestRegressor
    from sklearn.metrics import r2_score, mean_squared_error, mean_absolute_error
    from sklearn.model_selection import GroupKFold

    work = df[["Serie", "Sysser", "Fn", "Rn", RESPONSE_COLUMN]].copy()
    for c in ["Fn", "Rn", RESPONSE_COLUMN]:
        work[c] = pd.to_numeric(work[c], errors="coerce")
    work = work.dropna(subset=["Fn", "Rn", RESPONSE_COLUMN]).copy()

    key_to_idx = {
        (str(r["Serie"]), str(r["Sysser"])): i
        for i, (_, r) in enumerate(hull_df[["Serie", "Sysser"]].iterrows())
    }
    work["hull_index"] = [
        key_to_idx.get((str(a), str(b)), -1)
        for a, b in zip(work["Serie"], work["Sysser"])
    ]
    work = work[work["hull_index"] >= 0].reset_index(drop=True)

    groups = work["hull_index"].to_numpy()
    ...
    predictions = {
        "Baseline": np.full(len(work), np.nan),
        "Isomap_3D": np.full(len(work), np.nan),
        "Isomap_4D": np.full(len(work), np.nan),
    }

    Xgeom = np.asarray(X_hull, dtype=float)

    for fold, (train_idx, test_idx) in enumerate(gkf.split(work, groups=groups), 1):
        train_hulls = np.unique(groups[train_idx])
        test_hulls = np.unique(groups[test_idx])

        # Fit geometric scaling using training hulls only.
        scaler_fold = StandardScaler().fit(Xgeom[train_hulls])
        Xtr_geom = scaler_fold.transform(Xgeom[train_hulls])
        Xte_geom = scaler_fold.transform(Xgeom[test_hulls])

        # Map hull IDs to local row positions in the training/test embedding.
        tr_pos = {h: j for j, h in enumerate(train_hulls)}
        te_pos = {h: j for j, h in enumerate(test_hulls)}

        emb3 = Isomap(n_neighbors=min(10, len(train_hulls) - 2), n_components=3)
        emb4 = Isomap(n_neighbors=min(10, len(train_hulls) - 2), n_components=4)
        Ztr3 = emb3.fit_transform(Xtr_geom)
        Zte3 = emb3.transform(Xte_geom)
        Ztr4 = emb4.fit_transform(Xtr_geom)
        Zte4 = emb4.transform(Xte_geom)

        train_records = work.iloc[train_idx]
        test_records = work.iloc[test_idx]

        def record_geom_coords(records, Z, pos):
            return np.vstack([Z[pos[int(h)]] for h in records["hull_index"].to_numpy()])

        Xtr_base = train_records[["Fn", "Rn"]].to_numpy(float)
        Xte_base = test_records[["Fn", "Rn"]].to_numpy(float)
        Xtr3 = np.hstack([Xtr_base, record_geom_coords(train_records, Ztr3, tr_pos)])
        Xte3 = np.hstack([Xte_base, record_geom_coords(test_records, Zte3, te_pos)])
        Xtr4 = np.hstack([Xtr_base, record_geom_coords(train_records, Ztr4, tr_pos)])
        Xte4 = np.hstack([Xte_base, record_geom_coords(test_records, Zte4, te_pos)])

        ytr = train_records[RESPONSE_COLUMN].to_numpy(float)

        for name, Xtr, Xte in [
            ("Baseline", Xtr_base, Xte_base),
            ("Isomap_3D", Xtr3, Xte3),
            ("Isomap_4D", Xtr4, Xte4),
        ]:
            ...
            model.fit(Xtr, ytr)
            predictions[name][test_idx] = model.predict(Xte)

    y = work[RESPONSE_COLUMN].to_numpy(float)
    rows = []
    baseline_r2 = np.nan
    for name, pred in predictions.items():
        ...
        rows.append({
            ...
            "Inputs": {
                "Baseline": "Fn, Rn",
                "Isomap_3D": "Fn, Rn, Isomap-1..3",
                "Isomap_4D": "Fn, Rn, Isomap-1..4",
            }[name],
            "CV_folds": n_splits,
            "N_test_records": int(m.sum()),
            "N_test_hulls": int(len(unique_groups)),
            "CV_R2": r2,
            "RMSE": rmse,
            "MAE": mae,
            "Delta_R2_vs_baseline": 0.0 if name == "Baseline" else r2 - baseline_r2,
        })

    # A second, hull-aggregated table reduces sensitivity to the unequal
    # number of repeated records per hull.
    hull_pred_rows = []
    work2 = work.copy()
    for name, pred in predictions.items():
        work2[f"pred_{name}"] = pred
    for name in predictions:
        g = work2.groupby("hull_index", sort=False).agg(
            y_true=(RESPONSE_COLUMN, "mean"),
            y_pred=(f"pred_{name}", "mean"),
        ).dropna()
        if len(g) >= 2:
    ...
    return pd.DataFrame(rows), pd.DataFrame(hull_pred_rows)
...
def run_v6_paper_layer(df, hull_df, X_hull, X_scaled, pca, pca_scores, intrinsic_df, series_df,
                       ...
                       jackknife_summary, subsample_summary,
                       output_dir, image_dir=None, seed=42, interactive=True):
    """V6.1 publication layer: nonlinear manifold first, PCA supplementary."""
    ...
    print("\
" + "="*80)
    print("V6.1 PAPER OUTPUT — 3-D MANIFOLD VISUALIZATION FIRST")
    print("="*80)

    # -------------------------------------------------------------------------
    # Publication tables
    # -------------------------------------------------------------------------
    # These tables reorganize already-computed results into compact
    # computer-science/ML-paper style tables. No new scientific estimator is
    # introduced here; the only additional computation is the explicit
    # hull-level cross-validated hydrodynamic mapping below.
    overview = pd.DataFrame([
        ["Experimental records", len(df)],
        ["Independent hulls", len(hull_df)],
        ["Geometry dimensions", X_hull.shape[1]],
    ...
    # 16D variable definition table.
    _paper_save_table(
        _make_geometry_descriptor_table(),
        table_dir / "Table02_geometry_descriptors.csv"
    )

    # Descriptive statistics are retained as a separate table.
    geom_stats = X_hull.describe().T.reset_index().rename(columns={"index": "Variable"})
    _paper_save_table(geom_stats, table_dir / "Table03_geometry_statistics.csv")

    # Core intrinsic-dimensionality table.
    _paper_save_table(
        _make_intrinsic_dimension_table(intrinsic_df),
        table_dir / "Table04_intrinsic_dimension.csv"
    )

    # Series-level diagnostic table.
    _paper_save_table(
        _make_series_id_table(series_df),
        table_dir / "Table05_series_ID_diagnostics.csv"
    )

    # Unified robustness table.
    _paper_save_table(
        _make_robustness_table(jackknife_summary, subsample_summary, lvo_df),
        table_dir / "Table06_robustness_analysis.csv"
    )

    # Preserve full-resolution machine-readable analyses in the paper package.
    _paper_save_table(jackknife_df, table_dir / "TableS1_jackknife_hull_full.csv")
    _paper_save_table(subsample_df, table_dir / "TableS2_subsampling_full.csv")
    _paper_save_table(lvo_df, table_dir / "TableS3_leave_one_variable_out_full.csv")

    for src, dst in {
        "06_hull_pearson_correlation.csv": "TableS4_Pearson_correlation.csv",
        "07_hull_spearman_correlation.csv": "TableS5_Spearman_correlation.csv",
        "08_hull_mutual_information.csv": "TableS6_mutual_information.csv",
        "17_geometry_distance_summary.csv": "TableS7_geometry_distance_summary.csv",
        "19_hull_response_summary.csv": "TableS8_hull_response_summary.csv",
    }.items():
        p = output_dir / src
        if p.exists():
            _paper_save_table(pd.read_csv(p), table_dir / dst)
    ...
    # Nonlinear embedding quality table.
    q = pd.concat([
        isomap_df.assign(method_family="Isomap"),
        lle_df.assign(method_family="LLE")
    ], ignore_index=True)
    _paper_save_table(q, table_dir / "Table07_embedding_quality.csv")

    if iso3 is not None and iso4 is not None:
        d3 = pd.DataFrame(iso3, columns=["d1", "d2", "d3"])
        d4 = pd.DataFrame(iso4, columns=["d1", "d2", "d3", "d4"])
        extra = pd.DataFrame({
            "Metric": [
                "Isomap 3D coordinate variance sum",
                "Isomap 4D coordinate variance sum"
            ],
            "Value": [
                float(d3.var().sum()),
                float(d4.var().sum())
        ...
        _paper_save_table(extra, table_dir / "Table08_3D_4D_coordinate_diagnostic.csv")

    # -------------------------------------------------------------------------
    # Key new experiment: hull-level cross-validated hydrodynamic mapping.
    # This is the only substantive new computation added in V6.1.
    # -------------------------------------------------------------------------
    cv_record, cv_hull = _run_hydrodynamic_mapping_cv(
        df, hull_df, X_hull, n_splits=5, random_state=seed
    )
    _paper_save_table(cv_record, table_dir / "Table09_hydrodynamic_mapping_CV.csv")
    _paper_save_table(cv_hull, table_dir / "Table10_hydrodynamic_mapping_hull_CV.csv")
...
".join(tex),encoding="utf-8"); print("[FILE OK] figures.tex")
    ...
    report=["REAL SAILBOAT HULL MANIFOLD — V6.1 PAPER OUTPUT REPORT","="*78,f"Experimental records: {len(df)}",f"Independent hulls: {len(hull_df)}",f"Geometry dimensions: {X_hull.shape[1]}","",
            ...
            "The transparent 3-D envelope is a density-support visualization, not a fitted exact manifold.","Hydrodynamic residual adjustment is descriptive rather than causal.","Hydrodynamic mapping CV uses hull-level GroupKFold and fold-fitted Isomap; baseline uses Fn,Rn and augmented models add 3D/4D Isomap coordinates.","",
...
def main():
    parser = argparse.ArgumentParser(
        description="V6.1 intrinsic-dimension-first real sailboat hull manifold analysis with publication-grade 3D visualization."
    )
    ...
    print("\n" + "=" * 80)
    print("REAL SAILBOAT HULL DATASET AUDIT V6.1")
    print("=" * 80)
